...
ls.diag(m1)$hat
hatvalues(m1)
PalmBeach <- mutate(PalmBeach, lev=hatvalues(m1))
filter(PalmBeach, hatvalues(m1) > 3*2/67)
```
...
```{r}
PalmBeach <- mutate(PalmBeach, std=rstandard(m1), stud=rstudent(m1))
filter(PalmBeach, hatvalues(m1) > 3*2/67 | abs(std) > 3 | abs(stud) > 3)
```
...
```{r}
PalmBeach <- mutate(PalmBeach, cook=cooks.distance(m1))
filter(PalmBeach, County %in% c("PALM BEACH", "DADE", "ORANGE"))
```
