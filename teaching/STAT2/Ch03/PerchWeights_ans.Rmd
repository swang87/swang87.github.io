...
```{r}
m1 <- lm(Weight ~ Length*Width, data=Perch)
summary(m1)
m1a <- lm(Weight ~ Length + Width + Length:Width, data=Perch)
summary(m1a)
```
...
```{r}
m2 <- lm(Weight ~ Length*Width + I(Length^2) + I(Width^2), data=Perch)
summary(m2)
```
...
```{r}
m0 <- lm(Weight ~ Length + Width, data=Perch)
summary(m0)
summary(m1)
```
