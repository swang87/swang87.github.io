...
```{r dev = 'svg', fig.width = 8, fig.height = 4.5, echo =F}
library(ggplot2)
set.seed(5)
y <- rep(c(0,3), each=50) 
x <- 1:100
...
```{r dev = 'svg', fig.width = 8, fig.height = 5, echo =F}
y <- rep(c(0,1), each=50) 
x <- 1:100
z <- data.frame(x=x, y=y + rnorm(100), z= y)
ggplot(z, aes(x=x, y=y)) + geom_point() +
...
tmp3 <- data.frame(z=tmp2[[1]]$pmean, x = rep(1:N, each=N), y = rep(1:N, times=N))
ggplot(tmp3, aes(x,y)) + geom_tile(aes(fill = z), color="white") +
  scale_fill_gradient(low="white", high="steelblue")
...
plot(g, layout=lay, vertex.size=80, edge.width=4, rescale=F, 
      ylim=c(0,4), xlim=c(0,4), vertex.label.cex=2)
```
...
```{r fig.width = 6, fig.height = 5, echo =F, results='hide'}
#writeJPEG(matrix(tmp$pboundary, dim(x)[1], dim(x)[2]), "~/Desktop/PPMGraph/JSM2013/surelet-gray1-out-bound.jpg")
```
...
V(g)$name <- round(y,1)
y.rounded <- round(y,1)
y.scale <- as.character(seq(min(y.rounded), max(y.rounded), by=0.1))
cols <- rev(heat.colors(length(y.scale)))
lay <-layout.fruchterman.reingold(g, niter=100000)
V(g)$color <- cols[match(as.character(y.rounded), y.scale)]
...
tmp <- ppm(y, adj2, a0=0.4, p1=1, burnin=500, mcmc=500)[[1]]
V(g2)$name <- round(tmp$pmean,1)
tmp.rounded <- round(tmp$pmean,1)
tmp.scale <- as.character(seq(min(tmp.rounded), max(tmp.rounded), by=0.1))

cols <- rev(heat.colors(length(tmp.scale)))
V(g2)$color <- cols[match(as.character(tmp.rounded), tmp.scale)]
